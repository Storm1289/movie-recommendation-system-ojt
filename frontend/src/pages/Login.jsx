...
export default function Login() {
    const { login, signup, loginWithGoogle, loginWithFacebook, continueAsGuest } = useApp();
    ...
    useEffect(() => {
        ...
        loadScript('https://accounts.google.com/gsi/client', GOOGLE_SCRIPT_ID)
            .then(() => {
                ...
                window.google.accounts.id.initialize({
                    ...
                    callback: ({ credential }) => {
                        ...
                        loginWithGoogle({ credential })
                            ...
                            .catch((err) => {
                                setError(err?.response?.data?.detail || 'Unable to sign in with Google right now');
                            })
                            .finally(() => {
                                setSocialLoading('');
                            });
    ...
    useEffect(() => {
        ...
        window.fbAsyncInit = initialiseFacebook;

        loadScript('https://connect.facebook.net/en_US/sdk.js', FACEBOOK_SCRIPT_ID)
            .then(() => {
                initialiseFacebook();
            })
            .catch(() => {
                if (!isCancelled) {
                    setError('Facebook sign-in could not be loaded.');
                }
            });
        ...
        window.FB.login(
            async (response) => {
                ...
                loginWithFacebook({ access_token: accessToken })
                    ...
                    .catch((err) => {
                        setError(err?.response?.data?.detail || 'Unable to sign in with Facebook right now');
                    })
                    .finally(() => {
                        setSocialLoading('');
                    });
            ...
            <div className="relative z-10 w-full max-w-md px-6">
                ...
                <div className="bg-surface-dark/80 backdrop-blur-xl border border-white/10 rounded-2xl p-8 shadow-2xl">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        <div className="min-h-[44px] rounded-xl border border-slate-700 bg-bg-dark flex items-center justify-center px-3">
                            {googleConfigured ? (
                                <div className="relative w-full flex items-center justify-center">
                                    <div
                                        ref={googleButtonRef}
                                        className={`w-full overflow-hidden rounded-full ${googleReady ? '' : 'opacity-60'}`}
                                    />
                                    {googleBusy && (
                                ...
                                <button
                                    type="button"
                                    onClick={() => setError('Set VITE_GOOGLE_CLIENT_ID in the frontend and GOOGLE_CLIENT_ID in the backend to enable Google sign-in.')}
                                    className="w-full flex items-center justify-center gap-2 text-slate-300 text-sm font-semibold"
                                >
                        ...
                        <button
                            ...
                            disabled={!facebookConfigured || !facebookReady || Boolean(socialLoading)}
                            className="flex items-center justify-center gap-2 bg-bg-dark border border-slate-700 rounded-xl py-2.5 px-4 text-slate-300 hover:text-white hover:border-slate-500 transition-all text-sm font-semibold disabled:opacity-50 disabled:cursor-not-allowed"
                        >
                            {facebookBusy ? (
                                <div className="animate-spin w-5 h-5 border-2 border-white border-t-transparent rounded-full" />
                            ) : (
                                <>
                                    <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M24 12.073C24 5.405 18.627 0 12 0S0 5.405 0 12.073c0 6.024 4.388 11.018 10.125 11.927v-8.437H7.078v-3.49h3.047V9.412c0-3.007 1.792-4.669 4.533-4.669 1.313 0 2.686.235 2.686.235v2.953H15.83c-1.49 0-1.955.93-1.955 1.885v2.257h3.328l-.532 3.49h-2.796V24C19.612 23.091 24 18.097 24 12.073z" /></svg>
                                    {facebookConfigured ? 'Facebook' : 'Facebook Setup Required'}
                                </>
                            )}
                        </button>
                    </div>
                    ...
                    {!facebookConfigured && (
                        <p className="mt-3 text-xs text-slate-500">
                            Set `VITE_FACEBOOK_APP_ID` in the frontend and `FACEBOOK_APP_ID` / `FACEBOOK_APP_SECRET` in the backend to enable Facebook sign-in.
                        </p>
                    )}
