...
function LoginInner() {
    ...
    const doGoogleLogin = useGoogleLogin({
        onSuccess: (codeResponse) => {
            ...
            // We pass the access_token to the backend, which we will verify via userinfo
            loginWithGoogle({ credential: codeResponse.access_token })
                .then(() => { navigate('/home'); })
    ...
    const facebookStatus = facebookConfigured
        ? facebookReady
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        <div className="space-y-2">
                            <button
                                ...
                                disabled={!googleConfigured || Boolean(socialLoading)}
                                className="w-full flex items-center justify-center gap-2 bg-bg-dark border border-slate-700 rounded-xl py-2.5 px-4 text-slate-300 hover:text-white hover:border-slate-500 transition-all text-sm font-semibold disabled:opacity-50 disabled:cursor-not-allowed"
                            >
                                {googleBusy ? (
                                    <div className="animate-spin w-5 h-5 border-2 border-white border-t-transparent rounded-full" />
                                ) : (
                                    <>
                                        ...
                                        {googleConfigured ? 'Google' : 'Google Unavailable'}
                                    </>
                                )}
                            </button>
                            <p className="px-1 text-center text-[11px] font-medium text-slate-500">
                                {googleStatus}
                            </p>
                        </div>
                            ...
                            >
                                {facebookBusy ? (
                                    ...
                                    <>
                                        ...
                                        {facebookConfigured ? 'Facebook' : 'Facebook Coming Soon'}
                                    </>
                                )}
                            </button>
                            <p className="px-1 text-center text-[11px] font-medium text-slate-500">
